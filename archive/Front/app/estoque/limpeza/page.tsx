...
export default function EstoqueLimpezaPage() {
  ...
  const baseFiltered = useMemo(() => {
    ...
    if (!q) return insumos;
    return insumos.filter((i: any) => String(i.nome).toLowerCase().includes(q));
  }, [insumos, query]);
        ...
        <main className="flex-1 p-0 md:p-6 md:pb-6 overflow-hidden">
          <div className="h-full w-full bg-gray-50 text-gray-900 md:rounded-3xl p-8 overflow-y-auto shadow-2xl shadow-black/50">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Estoque - Limpeza</h1>
                <p className="text-gray-500 mt-1">Gerencie insumos de limpeza, unidades e quantidades.</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                <div className="text-gray-500 text-sm">Baixo estoque (&lt; 30)</div>
                <div className="text-amber-500 font-bold text-2xl">{filtered.filter((i:any) => Number(i.quantidade) < 30).length}</div>
              </div>
              <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
              ...
              <section className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    { label: "Listar Produtos", href: "/estoque/limpeza" },
                    { label: "Baixo estoque", href: "/estoque/limpeza?status=low" },
            ...
            {loading ? (
              ...
              <Table caption="Estoque de itens de limpeza" variant="light">
                <Thead>
                  <Th>
                    <button
                ...
                <Tbody>
                  {paginated.map((item: any) => {
                    const low = Number(item.quantidade) < 30;
                    return (
                      <Tr key={item.id}>
