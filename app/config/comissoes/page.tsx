              ...
              <tbody>
                {loading ? (
                  ...
                  (faixas || []).map((faixa: any, idx: number) => (
                    <tr key={idx}>
                      <td className="px-4 py-2">{faixa.loja}</td>
                      <td className="px-4 py-2">{faixa.faixa}</td>
                      <td className="px-4 py-2">{faixa.volume}</td>
          ...
          <table className="min-w-full bg-gray-900 rounded-xl shadow border border-gray-800">
            ...
            <tbody>
              {loading ? (
                ...
                faixas.map(faixa => (
                  <tr key={faixa.id} className="transition hover:bg-gray-800 focus-within:bg-gray-800">
                    <td className="px-4 py-2 font-bold text-emerald-400">{faixa.nome}</td>
                    <td className="px-4 py-2">{faixa.porcentagem_comissao ?? "-"}%</td>
                    <td className="px-4 py-2">R$ {faixa.meta_venda?.toLocaleString() ?? "-"}</td>
                    <td className="px-4 py-2">
                      <button className="bg-sky-600 hover:bg-sky-700 text-white font-bold py-1 px-3 rounded shadow transition active:scale-95" aria-label="Editar Faixa">Editar</button>
                    </td>
                  </tr>
                ))
              )}
